...
- `UNINSTALL_FA4` (default `1`) — on the `reason` target, uninstalls `flash-attn-4` during the image build. Transformer Engine then uses FA2, which also runs on GPUs older than sm90 (for example A100). Docker builds cannot see the GPU, so `install.sh` cannot make this choice by itself. Set it to `0` to keep FA4 for an image that runs only on Hopper or newer; the published FA4 image carries a `-fa4` tag suffix.
