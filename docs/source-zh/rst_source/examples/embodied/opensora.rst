...
作为世界模型，OpenSora 理论上可在一致的接口下拟合任意任务。RLinf 目前提供两个 LIBERO 套件的权重与初始化数据：
...
.. list-table::
   ...
   * - 奖励
     - 由世界模型的奖励判定器给出，范围 ``[0, 1]``。
   * - 任务提示
     - 自然语言任务描述。

不同于传统仿真器，OpenSora 没有 ``reset()``：它需要初始帧与任务描述，因此需提前下载初始化数据集并在配置中指定路径。
