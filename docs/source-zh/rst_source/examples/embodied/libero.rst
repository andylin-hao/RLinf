...
`LIBERO <https://libero-project.github.io>`__ 是一个面向**终身机器人学习（lifelong robot
...
:doc:`支持的加速器 <../../tutorials/accelerators/index>` 教程。

任务套件
--------

LIBERO 提供**五个任务套件，共 130 个任务**，从单步抓取放置到长程多步场景。通过配置名选择套件；
...
**一图速览** —— 在更具挑战性的 LIBERO-Pro / LIBERO-Plus 扰动套件上检验模型泛化能力。
...
.. grid:: 2 4 4 4
   ...
   .. grid-item-card:: 任务
      ...
      Pro：4 个扰动维度 · Plus：10,030 个任务 / 7 个维度
...
| **你将完成：** 安装套件（``--env liberopro`` / ``liberoplus``）→（LIBERO-Plus）下载资产包 → 复用 LIBERO 基座模型 → 通过 ``LIBERO_TYPE=pro|plus`` 训练 / 评测。
| **前置条件：** :doc:`安装 </rst_source/start/installation>` · 复用标准 LIBERO 基座检查点。
