...
在 BEHAVIOR 家居任务上用 PPO/GRPO 对 VLA 进行强化学习微调。
...
| **你将完成：** 安装（IsaacSim）→ 下载资产与基座模型 → 运行 ``run_embodiment.sh`` → 观察 ``env/success_once``。
...
.. warning::
   ...
   另外，如果您的GPU没有Ray Tracing能力（例如A100、H100），BEHAVIOR的渲染质量会非常差，画面可能会出现严重的马赛克或模糊。因此，我们建议使用具有Ray Tracing能力的GPU（如RTX 30系列、40系列或更高）来获得最佳的视觉效果和训练体验。
...
您可以灵活配置 env、rollout 和 actor 组件的 GPU 数量。因为 Behavior 环境的步进非常慢，通常建议给 env 分配更多 GPU 来提升性能，但这也会增加显存占用。一个 Behavior 环境约占 10 GiB 显存，您可以根据自己的 GPU 显存大小调整每 GPU 的环境进程数量（如 RTX 4090 24G 卡，可以将 num_env_subprocess 设置为 2，每卡开启 2 个环境进程）。

另外，Behavior 环境支持单进程内部开启多向量化环境，因此 total_num_envs 可以是 Behavior 总进程数量（即env GPU 数量和 num_env_subprocess 的乘积）的整数倍。例如，env GPU 数量为 4，num_env_subprocess 为 2，则 total_num_envs 可以设置为 8、16、32 等。如果 pipeline_stage_num == 2，则 total_num_envs 至少应是总进程数量的 2 倍（16）。
...
- ``num_env_subprocess``：
  ...
  分别承载 Isaac/OmniGibson 仿真（见 ``behavior_env.py`` 中的 ``BehaviorProcess``）。
