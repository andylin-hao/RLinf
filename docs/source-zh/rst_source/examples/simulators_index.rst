...
如果你的出发点是 "我想在基准 *X* 上训练"，那这里就是合适的入口。若以模型为主线（pi₀、GR00T 等）请参考 :doc:`vla_wam_index`\ ；Franka 真机请参考 :doc:`franka_index`\ ，其他真机部署请参考 :doc:`real_world_index`\ 。如需在 AMD ROCm 或 Ascend CANN 加速器上运行 LIBERO，请参阅 :doc:`支持的加速器 <../tutorials/accelerators/index>` 教程章节。
