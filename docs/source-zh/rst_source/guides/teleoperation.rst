...
遥操作时，操作者可以在 rollout 途中替换策略动作，用来采集示教、挽救失败动作或运行
DAgger。装置可以只有机器人旁的一台设备，也可以把多台设备分散接到不同机器上。设备
数量和位置不同，使用的仍是同一套设备与放置模型。
...
每台设备为自己能驱动的具名部件生成动作。配置值写成列表时，多台设备各管一部分：
...
这套灵巧手装置中，操作球控制机械臂，数据手套控制手。只有按住 SpaceMouse 的第二个键，
手套才会接管；松开后，手停在上一个位姿。
...
binding 声明的部件若不在机器人上，group 会跳过该部件。设备最终匹配不到任何部件，
或者两台设备声明了同一个部件，构建装置时都会报错。
...
只有跟随明显滞后时才需要直推模式。开启后，``env.step`` 不再下发关节目标；装置若配置
