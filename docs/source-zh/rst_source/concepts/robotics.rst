...
以自己的名字出现；而驱动多个组件的总线不是其中任何一个，读它也没有任何意义。
``Connection`` 标记的正是后者。它是一个独立的类型，而不是「拒绝被读取的部件」：
机器人根本无法把 ``Connection`` 组合进部件树。
...
部件之下还有一个更小的概念。``Endpoint`` 指机器人在某台机器上打开、之后再关闭的
任何东西：它有位置，因而可以在此声明、在彼构建，也有生命周期。``RobotPart`` 是
「读起来有意义」的 ``Endpoint``；``Connection`` 则是读起来没有意义的那种——这正是
两者互为兄弟、而非一方继承另一方的原因。
...
.. list-table::
   ...
   * - 抽象
     - 含义
   * - ``Endpoint``
     - 机器人打开又关闭的任何东西：只有位置和生命周期两件事。它定义 ``connect``、
       ``disconnect``、``reset`` 和 ``at()``。部件和连接都是 ``Endpoint``。
   * - ``RobotPart``
     - 可以读取的 ``Endpoint``，这也正是它成为机器人物理组件的原因。它增加了
