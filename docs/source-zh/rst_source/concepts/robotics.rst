...
episode 之间，任务只提出要求，而不编写具体动作。``hold()`` 让机械臂停在当前位置，不再追踪 policy 最后下发的目标；``clear(distance=..., qpos=...)`` 让末端脱离正在接触的物体：以末端位姿驱动的机械臂抬起相应距离，以关节目标驱动的机械臂则转到为此指定的关节配置；``go_home(Home(...))`` 前往任务指定的等待位置，并在未到达容差范围内时重新下发；``unwind(qpos)`` 把关节回到一个已知配置。任务会同时给出两种等待位置的写法，每条机械臂各取自己能用的那一种，因此插销任务在 Franka 和 GimArm 上是同一个任务。

机械臂如何走完这段距离，同样由它自己决定。``move_to()`` 通过一系列等间隔的目标点前进，适合只在收到指令时才移动的机械臂：Franka 会限制单条指令能带它走多远，然后等待下一条，因此把整条路径逐点送过去是调用方的职责。如果某台机械臂的控制器会按自己的节拍持续朝目标前进，需要的恰好相反，它应当重写 ``move_to()``，把目标位姿一次交给控制器。Turtle2 就是如此：它的控制器以自身的速度把机械臂送过去，并在自身的容差内停下；如果再喂给它一条路径，只会让一个正在朝目标前进的循环不断被换掉目标。无论哪种情况，``clear()`` 和 ``go_home()`` 所提出的要求都不变。
