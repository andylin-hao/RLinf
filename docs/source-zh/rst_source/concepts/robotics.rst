...
任务或 env 操作真机时，不应直接调用具体 driver，而应通过机器人接口访问所需零部件，并以统一方式检查状态、管理连接和读写数据。机器人接口为机械臂、末端执行器、相机和移动底盘等零部件定义稳定的名称与访问路径，观测和动作也沿用这些路径。只要零部件路径保持不变，更换 backend 或调整部署节点就无需修改任务侧的调用代码。
