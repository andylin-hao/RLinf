...
下面的示例（改写自 ``ClusterConfig`` 的文档字符串）展示了一个具有异构硬件的集群：
...
.. code-block:: yaml
	...
	cluster:
	  ...
	  component_placement:
	    actor:
	      node_group: a800
	      placement: 0-63           # 加速卡（GPU）硬件编号
	    rollout:
	      node_group: 4090
	      placement: 0-63           # 加速卡（GPU）硬件编号
	    env:
	      node_group: franka
	      placement: 0-1            # 机器人硬件编号
	    agent:
	      node_group: node
...
* ``node_groups`` —— 每一项定义了一个 **节点组（node group）**，
  ...
  - ``node_ranks``：属于该节点组的全局节点编号列表或范围。
    在示例中，``a800`` 覆盖 ``0-7``，``4090`` 覆盖 ``8-15``，
    ``franka`` 覆盖 ``16-17``。

  - ``python_interpreter_path``（可选）：在该节点组上启动 worker 时使用的
    Python 解释器路径。如果未指定，则默认使用在该节点上启动 Ray 时所用的解释器。

  - ``env_vars``（可选）：在该节点组中的节点上需要设置的一组环境变量。
    列表中的每一项是一个只包含单个键值对的字典，例如
    ``GLOO_SOCKET_IFNAME: "eth0"``。
