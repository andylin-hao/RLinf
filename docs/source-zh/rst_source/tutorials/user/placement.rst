...
* ``isolate_accelerator`` – 是否通过设置 ``CUDA_VISIBLE_DEVICES`` 来限制进程只看到分配给它的 GPU（默认为 ``True``）  
...
此部署策略允许为每个进程分配任意的 GPU ID，指定为一个 GPU ID 列表的列表，每个 GPU ID 列表包含分配给一个工作进程的全局 GPU ID。

所需输入
~~~~~~~~~~~~~~~~~

* ``gpu_id_lists`` – 一个全局 GPU ID 列表的列表，每个内部列表指定分配给一个工作进程的 GPU。

分配原则
~~~~~~~~~~~~~~~~~~~~~
调度器遍历提供的 ``gpu_id_lists``，对于每个内部列表：
1. 将指定的全局 GPU ID 分配给一个工作进程
2. 节点 ID 由该内部列表中的第一个 GPU ID 决定
3. 本地 GPU ID 相对于该节点进行计算

目的
~~~~~~~~~~~~~
此策略提供最大的灵活性，允许用户精确定义每个工作进程应使用的 GPU，无论是否连续或是否具有特定间隔。这在因硬件拓扑或其他限制需要特定 GPU 分配的场景中特别有用。
