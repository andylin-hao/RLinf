...
As a world model, OpenSora can in principle fit any task behind a consistent
interface. RLinf currently ships weights and init data for two LIBERO suites:
...
.. list-table::
   ...
   * - Field
     - Description
   * - Observation
     - RGB frames generated by the world model, ``[B, 256, 256, 3]``.
   * - Action
     - 7-D continuous actions — 3-D end-effector position (x, y, z), 3-D rotation (roll, pitch, yaw), gripper open/close — normalized and tokenized.
   * - Reward
     - Provided by the world model's reward classifier, in ``[0, 1]``.
   * - Task prompt
     - Natural-language task description.

Unlike a traditional simulator, OpenSora has no ``reset()``: it requires initialization
frames and a task description, so you download an initialization dataset and point the
config at it.
...
**Option 1: Docker image** — image tag ``agentic-rlinf0.2-opensora``:
...
.. code:: bash
   ...
   docker run -it --rm --gpus all \
      ...
      # Mainland China mirror: docker.1ms.run/rlinf/rlinf:agentic-rlinf0.2-opensora
