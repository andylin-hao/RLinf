...
As a world model, Wan can in principle fit many task settings behind a consistent
interface. RLinf currently ships weights and init data for three LIBERO suites:
...
.. list-table::
   ...
   * - Reward
     - Predicted by the world model's reward classifier, in ``[0, 1]``.
   * - Task prompt
     - Natural-language task description.

Unlike a traditional simulator, Wan has no ``reset()``: it requires initialization frames
and a task description, so you download an initialization dataset and point the config at it.
...
**Option 1: Docker image** — image tag ``agentic-rlinf0.2-wan``:
...
.. code:: bash
   ...
   docker run -it --rm --gpus all \
      ...
      # Mainland China mirror: docker.1ms.run/rlinf/rlinf:agentic-rlinf0.2-wan
