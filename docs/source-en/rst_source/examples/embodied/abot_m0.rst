...
.. list-table::
   ...
   * - LIBERO-10
     ...
     - PPO training from the ABot-M0 RL baseline checkpoint.
   * - LIBERO-10
     - ``libero_10_ppo_abot_m0``
     - Evaluation of an ABot-M0 SFT checkpoint with evaluation overrides.
   * - LIBERO-Plus
     - ``libero_10_plus_ppo_abot_m0``
     - PPO training after installing the additional LIBERO-Plus assets.
...
.. list-table::
   ...
   * - Field
     - Description
   * - Observation
     - LIBERO RGB observations and proprioceptive state consumed by ABot-M0 through the HuggingFace rollout wrapper.
   * - Action
     - Chunked continuous robot actions generated by the ABot-M0 action model.
   * - Reward
     - LIBERO task success and dense environment reward.
   * - Prompt
