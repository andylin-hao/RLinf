...
[.claude/skills/test-install/driver.py](.claude/skills/test-install/driver.py) —
...
python3 .claude/skills/test-install/driver.py list

# Show the exact install command + env vars + test configs for one combo:
python3 .claude/skills/test-install/driver.py resolve gr00t_n1d6 maniskill_libero

# Do the model/checkpoint paths an e2e config needs actually exist on disk?
python3 .claude/skills/test-install/driver.py check-paths libero_spatial_ppo_gr00t_n1d6

# Same check across every e2e config at once (great pre-flight / PR check):
python3 .claude/skills/test-install/driver.py check-all
```
...
```bash
python3 .claude/skills/test-install/driver.py run gr00t_n1d6 maniskill_libero \
...
python3 .claude/skills/test-install/driver.py install gr00t_n1d6 maniskill_libero \
    --venv /workspace/test-venvs/gr00t_n1d6 --dry-run
...
python3 .claude/skills/test-install/driver.py test realworld_dummy_sac_cnn \
...
python3 .claude/skills/test-install/driver.py resolve <model> <env>
# 2. The SFT checkpoint the e2e config points at is actually on this box:
python3 .claude/skills/test-install/driver.py check-paths <its_config>
# 3. Full build + test (preview with --dry-run, then drop it to run for real):
python3 .claude/skills/test-install/driver.py run <model> <env> \
