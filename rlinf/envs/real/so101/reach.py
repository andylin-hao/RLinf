...
"""SO-101 joint reach: hold a target configuration.

The smallest task that exercises the whole path -- compose the robot, read
it, command it, and score the result -- so a new arm can be brought up
before any manipulation task is written for it.
"""
...
class SO101ReachConfig(SO101RobotConfig):
    ...
    def __post_init__(self) -> None:
        ...
        if len(self.target_joint_qpos) != _DOF:
            raise ValueError(
                f"An SO-101 has {_DOF} arm joints, so 'target_joint_qpos' "
                f"needs {_DOF} values, got {len(self.target_joint_qpos)}."
            )
