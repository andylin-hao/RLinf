    ...
    if rank_config.lower() == "all":
        ...
        for rank_range in rank_ranges:
            ...
            try:
                ...
                raise ValueError(
                    f'Invalid rank format {rank_config} for rank type {rank_type}, expected format: "a,b,c-d" or "all"'
                )
            assert end_rank >= start_rank, (
                f"Start rank {start_rank} must be less than or equal to end rank {end_rank} in rank config {rank_config} for rank type {rank_type}."
            )
            if available_ranks is not None:
