...
def compute_rollout_metrics(data_buffer: dict) -> dict:
    ...
    if "rewards" in data_buffer:
        ...
        mean_rewards = torch.mean(rewards).to(dutils.current_device())
    ...
    if "advantages" in data_buffer:
        ...
        mean_adv = torch.mean(advantages).to(dutils.current_device())
        ...
        reduce_adv_tensor = torch.as_tensor(
            [-min_adv, max_adv], device=dutils.current_device(), dtype=torch.float32
        )
    ...
    if data_buffer.get("returns", None) is not None:
        ...
        mean_ret = torch.mean(returns).to(dutils.current_device())
        ...
        reduce_ret_tensor = torch.as_tensor(
            [-min_ret, max_ret], device=dutils.current_device(), dtype=torch.float32
        )
        torch.distributed.all_reduce(
