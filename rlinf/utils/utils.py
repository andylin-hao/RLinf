...
def setup_dst_ranks(
    batch_size: int, src_rank: int, src_world_size: int, dst_world_size: int
) -> list[tuple[int, int]]:
    """Compute destination ranks and transfer sizes for one source rank.

    Args:
        batch_size: Total batch size distributed across all source ranks.
        src_rank: Rank index in the source worker group.
        src_world_size: World size of the source worker group.
        dst_world_size: World size of the destination worker group.

    Returns:
        Ordered ``(dst_rank, size)`` tuples. ``size`` is the amount of batch
        data this source rank should send to that destination rank.
    """
