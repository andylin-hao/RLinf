...
"""Isolated image preprocessing adapted from the StreamingVLA OpenPI fork."""
...
def preprocess_observation_pytorch(
    ...
    *,
    ...
    """Apply the exact StreamingVLA training image augmentations."""
    if not set(image_keys).issubset(observation.images):
        raise ValueError(
            f"images dict missing keys: expected {image_keys}, "
        ...
        if train:
            ...
            if "wrist" not in key:
                ...
                if torch.abs(angle) > 0.1:  # Only rotate if angle is significant
                    ...
                    if grid.dtype != image.dtype:
                        grid = grid.to(dtype=image.dtype)
