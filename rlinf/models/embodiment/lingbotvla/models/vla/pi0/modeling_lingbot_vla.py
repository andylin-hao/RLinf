...
def eager_attention_forward(
    ...
    **kwargs,
    ...
    if attention_mask is not None:
        # 🚨 强制剔除防御性截断：如果掩码和 Key 长度不一致，直接抛出异常！
        if attention_mask.shape[-1] != key_states.shape[-2]:
            raise ValueError(
                f"🚨 Attention Mask 长度 ({attention_mask.shape[-1]}) 与 KV Cache 长度 ({key_states.shape[-2]}) 不匹配! 绝对是丢了视觉前缀!"
            )
...
class Qwen2DecoderLayer(nn.Module):
    def __init__(self, config: Qwen2Config, layer_idx: int):
        ...
        if config.norm_qkv:
            self.q_layernorm = Qwen2RMSNorm(
                self.self_attn.head_dim, eps=config.rms_norm_eps
            )
            self.k_layernorm = Qwen2RMSNorm(
                self.self_attn.head_dim, eps=config.rms_norm_eps
            )
        ...
        if compute_kqv:
            ...
            if ada_cond is not None:
                if old_adanorm:
                    ...
                    out_emb, after_gate = self.post_attention_layernorm(
                        out_emb, ada_cond
                    )
            else:
...
class AdaRMSNorm(nn.Module):
    def __init__(
        self, hidden_size, cond_dim, split_gate_liner, no_split_gate_liner, eps=1e-6
    ):
        """
        AdaRMSNorm: RMSNorm + FiLM
        """
...
class QwenvlWithExpertModel(PreTrainedModel):
    ...
    def __init__(self, config: QwenvlWithExpertConfig):
        ...
        self.qwen_expert = Qwen2ForCausalLM._from_config(
            self.config.qwen_expert_config, use_flash_attention_2=True
        )
        ...
        if self.config.enable_expert_vision:
            ...
            self.expert_visual_mlp = nn.Sequential(
                nn.Linear(
                    self.expert_visual.embed_dim, self.expert_visual.embed_dim * 2
                ),
                nn.GELU(),
                nn.Linear(
                    self.expert_visual.embed_dim * 2,
                    self.config.qwen_expert_config.hidden_size,
                ),
            )
    ...
    def get_image_features(
        ...
        split_sizes = (
            image_grid_thw.prod(-1) // self.qwenvl.visual.spatial_merge_size**2
        ).tolist()
        ...
        for layer_idx in range(num_layers):
            ...
            for i, hidden_states in enumerate(inputs_embeds):
                ...
                if i == 1:  # For action expert
                    query_state, key_state, value_state, gate = models[i].layers[
                        layer_idx
                    ](
                        hidden_states,
                        compute_kqv=True,
                        ada_cond=ada_cond,
                        norm_qkv=norm_qkv,
                        old_adanorm=self.config.old_adanorm,
                    )
                else:  # For VLM
...
class FlowMatching(nn.Module):
    def __init__(self, config):
        ...
        qwenvl_with_export_config.split_gate_liner = getattr(
            config, "split_gate_liner", False
        )
        qwenvl_with_export_config.no_split_gate_liner = getattr(
            config, "nosplit_gate_liner", False
        )
        qwenvl_with_export_config.separate_time_proj = getattr(
            config, "separate_time_proj", False
        )
        qwenvl_with_export_config.old_adanorm = getattr(config, "old_adanorm", False)
        qwenvl_with_export_config.final_norm_adanorm = getattr(
            config, "final_norm_adanorm", False
        )
        qwenvl_with_export_config.norm_qkv = getattr(config, "norm_qkv", False)
        self.qwenvl_with_expert = QwenvlWithExpertModel(qwenvl_with_export_config)
        self.config.proj_width = (
            qwenvl_with_export_config.qwen_expert_config.hidden_size
        )
    ...
    def embed_prefix(
        ...
        img_emb = einops.rearrange(
            img_emb, "(b n) l d -> b (n l) d", b=bsize
        )  # bsize = 24
        ...
        if self.use_depth_align and self.align_type == "query":
            align_masks = einops.repeat(
                img_masks, "b n -> b (n l)", l=self.num_task_tokens
            )
        ...
        if self.use_depth_align and self.align_type == "query":
            ...
            embs = torch.cat(
                [img_emb, align_embs, align_embs, align_embs, lang_emb], dim=1
            )
        ...
        if not vlm_causal:
            if self.use_depth_align and self.align_type == "query":
                att_masks = torch.zeros(
                    (
                        img_emb.size(0),
                        num_img_embs + 3 * self.num_task_tokens + num_lang_embs,
                    ),
                    device=device,
                    dtype=torch.bool,
                )
    ...
    def embed_suffix(self, state, noisy_actions, timestep, expert_imgs=None):
        ...
        if expert_imgs is not None:
            if expert_imgs.ndim == 5:
                ...
                bsize = 1
            expert_img_emb = self.qwenvl_with_expert.expert_visual.forward_features(
                expert_imgs
            )["x_norm_clstoken"].unsqueeze(1)
            expert_img_emb = self.qwenvl_with_expert.expert_visual_mlp(expert_img_emb)
            expert_img_emb = einops.rearrange(
                expert_img_emb, "(b n) l d -> b (n l) d", b=bsize
            )  # bsize = 24
            embs = torch.cat(
                [expert_img_emb, state_emb[:, None], action_time_emb], dim=1
            )
            num_expert_img_emb = expert_img_emb.shape[1]
            pad_masks = torch.ones(
                (bsize, action_time_dim + 1 + num_expert_img_emb),
                device=device,
                dtype=torch.bool,
            )
            att_masks = torch.zeros(
    ...
    def forward(
        ...
        if self.config.align_params != {}:
            loss_depth, depth_preds = self.depth_emb_forward(
    ...
    def predict_velocity(
        self, state, prefix_pad_masks, past_key_values, x_t, expert_imgs, timestep
    ):
    ...
    def make_att_2d_masks_with_query(self, att_2d_masks, prefix_len, img_masks):
        ...
        att_2d_masks[
            ...
            num_image_tokens * 3 + self.num_task_tokens * 0 : num_image_tokens * 3
            + self.num_task_tokens * 3,
        ] = False
        att_2d_masks[
            :,
        ...
        att_2d_masks[
            img_masks[:, 1],
            num_image_tokens * 3 + self.num_task_tokens * 1 : num_image_tokens * 3
            + self.num_task_tokens * 2,
            num_image_tokens * 1 : num_image_tokens * 2,
        ] = True
        att_2d_masks[
            img_masks[:, 2],
            num_image_tokens * 3 + self.num_task_tokens * 2 : num_image_tokens * 3
            + self.num_task_tokens * 3,
    ...
    def depth_emb_forward(self, hidden_states, depth_targets=None, img_masks=None):
        ...
        if self.align_type == "direct":
            ...
            align_embs = hidden_states[
                :, chunk_size * 3 : chunk_size * 3 + self.num_task_tokens * 3, :
            ]
            ...
            depth_preds = (
                self.depth_align_head(align_embs, depth_preds).contiguous().float()
            )
    ...
    def _emb_loss(self, emb_preds, emb_targets):
        if self.align_type == "direct":
            ...
            emb_preds = emb_preds.contiguous().view(
                S * self.llm_image_token_size * self.llm_image_token_size,
                emb_preds.shape[-1],
            )
            ...
            emb_targets = (
                emb_targets.view(
                    S, D, self.llm_image_token_size * self.llm_image_token_size
                )
                .permute(0, 2, 1)
                .contiguous()
            ...
            l1_loss = F.l1_loss(
                emb_preds.float(), emb_targets.float().detach(), reduction="none"
            )
            ...
            emb_preds_matrix = torch.matmul(
                emb_preds_norm, emb_preds_norm.transpose(0, 1)
            )
            emb_targets_matrix = torch.matmul(
                emb_targets_norm, emb_targets_norm.transpose(0, 1)
            )

            sim_loss = F.l1_loss(
                emb_preds_matrix.float(),
                emb_targets_matrix.float().detach(),
                reduction="none",
            )
