    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if compute_kqv:
            ...
            raise ValueError(
                f"Invaild Operation compute_kqv={compute_kqv} and output_atten={output_atten} with GemmaDecoderLayer in PI0"
            )
...
class GemmaModel(GemmaPreTrainedModel):
    ...
    def __init__(self, config: GemmaConfig, **kwargs):
        ...
        assert not (in_expert and in_vlm), (
            "expert and vlm cannot be True at the same time"
        )
        ...
        if in_expert:
            ...
            self.layers = nn.ModuleList(
                [
                    MpGemmaDecoderLayer(config, layer_idx)
                    for layer_idx in range(config.num_hidden_layers)
                ]
            )
        if in_expert:
...
class PaliGemmaWithExpertModel(PreTrainedModel):
    ...
    def __init__(self, config: PaliGemmaWithExpertConfig):
        ...
        self.gemma_expert = GemmaForCausalLM(
            config=config.gemma_expert_config, expert=True
        )
...
class PI0FlowMatching(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.config.initializer_range = getattr(
            paligemma_with_export_config.gemma_expert_config, "initializer_range", None
        )
        # projection layers
    ...
    def embed_prefix(
        ...
        # ipdb.set_trace()
        img_emb = self.paligemma_with_expert.embed_image(
            images
        )  # torch.Size([72, 3, 224, 224]) -> torch.Size([72, 256, 2048])
    ...
    def forward(
        ...
        pad_masks = torch.cat(
            [prefix_pad_masks, suffix_pad_masks], dim=1
        )  # 1,state_bs*(768+48+50+1)
        att_masks = torch.cat(
            [prefix_att_masks, suffix_att_masks], dim=1
        )  # 1,state_bs*(768+48+50+1)
        ...
        )  # torch.Size([state_bs, 768+48+50+1])
        ...
        (_, suffix_out), _ = self.paligemma_with_expert.forward(
            ...
            inputs_embeds=[
                prefix_embs,
                suffix_embs,
            ],  # bs_img,(768+48),2048  [state_bs, (50+1), 1024]
            use_cache=True,
            fill_kv_cache=True,
        )
